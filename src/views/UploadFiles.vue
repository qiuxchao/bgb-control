         ...
         <div class="msg-box">
            ...
            <el-dialog class="dialog" top="10vh" :visible.sync="showDetails">
                ...
                <div class="message" v-if="details">
                    ...
                    <p v-if="details.filePicPath"><strong >文件图片地址 : </strong>{{ details.filePicPath }}</p>
                    <p v-if="details.uploadTime"><strong>上传时间 : </strong>{{ details.uploadTime }}</p>
                </div>
            </el-dialog>
        </div>
    </section>
...
export default {
    ...
    methods: {
        handleCurrentChange(val) {
            ...
            this.selectPage = val;
            if(this.copyId != "") {
                this.$axios.post(`/upload/findAllByUserId?userId=${this.copyId}&pageindex=${val}`)
                    .then(res => {
                        console.log(res);
        ...
        handleDelete(index, data) {
            // 删除文件
            console.log(index, data);
            this.$axios.post("/upload/deleteByKeyId?ids="+data.id)
                .then(res => {
                    console.log(res);
                    this.$message({
                        message: "删除成功",
                    ...
                    this.$axios.post(`/upload/findAllByUserId?userId=${this.copyId}&pageindex=${this.selectPage}`)
                    .then(res => {
                        console.log(res);
                        this.data = res.data.data.list;
                        this.total = res.data.data.total;
        ...
        getData() {
            ...
            if(this.search == "") {
                ...
                this.selectPage = 1;
                this.$axios.post(`/upload/findAllByUserId?userId=${this.search}&pageindex=${1}`)
                    .then(res => {
                        console.log(res);
                        this.data = res.data.data.list;
                        this.total = res.data.data.total;
