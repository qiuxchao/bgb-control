        ...
        <div class="title">
         ...
         <div class="msg-box">
            ...
            <div class="space-info" v-if="dataFilter(data, keyword) != []">
                ...
                height="7.1rem"
                :stripe="true" >
                    <el-table-column label="文档名" prop="docName"></el-table-column>
                    <el-table-column label="文档类型" prop="docType"></el-table-column>
                    <el-table-column label="文档大小" prop="docSize"></el-table-column>
                    <el-table-column label="文档状态" prop="fileState"></el-table-column>
            ...
            <el-dialog class="dialog" top="10vh" :visible.sync="showDetails">
                <h2 class="msg-title">文档详情</h2>
                <div class="message" v-if="details">
                    <p v-if="details.id"><strong>文档id : </strong>{{ details.id }}</p>
                    <p v-if="details.userId"><strong>用户id : </strong>{{ details.userId }}</p>
                    <p v-if="details.docName"><strong>文档名 : </strong>{{ details.docName }}</p>
                    <p v-if="details.docType"><strong>文档类型 : </strong>{{ details.docType }}</p>
                    <p v-if="details.docPath"><strong>文档地址 : </strong>{{ details.docPath }}</p>
                    <p v-if="details.docSize"><strong>文档大小 : </strong>{{ details.docSize }}</p>
...
export default {
    ...
    methods: {
        handleCurrentChange(val) {
            ...
            this.selectPage = val;
            if(this.copyId != "") {
                this.$axios.post(`/document/stlectByIdPage?userId=${this.copyId}&pageindex=${val}`)
                    .then(res => {
                        console.log(res);
        ...
        handleDelete(index, data) {
            // 删除文件
            console.log(index, data);
            this.$axios.post("/document/deleteByKeyId?id="+data.id)
                .then(res => {
                    console.log(res);
                    this.$message({
                        message: "删除成功",
                    ...
                    this.$axios.post(`/document/stlectByIdPage?userId=${this.copyId}&pageindex=${this.selectPage}`)
                    .then(res => {
                        console.log(res);
                        this.data = res.data.data.list;
        ...
        getData() {
            ...
            if(this.search == "") {
                ...
                this.selectPage = 1;
                this.$axios.post(`/document/stlectByIdPage?userId=${this.search}&pageindex=${1}`)
                    .then(res => {
                        console.log(res);
                        this.data = res.data.data.list;
                        this.total = res.data.data.total;
