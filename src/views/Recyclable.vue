...
<script>
// 文件组件
import Files from '../components/RecyclableFiles'
// 文件夹组件
import Folders from '../components/RecyclableFolders'
// 图片组件
import Photos from '../components/RecyclablePhotos'
export default {
    name: "Folder",
    data() {
        return {
    ...
    methods: {
        searchFolder() {
            ...
            if(this.folder == "") {
                ...
                this.$axios.post(`/folder/selectRecovery?id=${this.folder}`)
                    .then(res => {
                        console.log(res);
        ...
        getData() {
            ...
            if(this.search == "") {
                ...
                this.$axios.post(`/folder/selectRecovery?userId=${this.search}`)
                    .then(res => {
                        console.log(res);
                        if(res.data.status == 500) {
                            this.$message.error("500: 服务器返回错误");
